...
export type APSSnap = {
  ...
  startDeg?: number
  endDeg?: number
  aSqVerts?: [ASV, ASV, ASV, ASV]
} // snapshot of a square at one face
...
export default class Render {
  ...
  globalHoveredSq: SquareID | null = null
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let r = 1; r < 4; r++) {
        ...
        for (let rr = 1; rr < 4; rr++) {
          ...
          rowVerts.push([lastRV[0] + cosDeg2 * SL, lastRV[1] + sinDeg2 * SL])
        }
        ...
        for (let rr = 0; rr < 3; rr++) {
          ...
          const sqVerts: PositionType[] = [
            ...
            r === 0
              ? rr === 0
                ? GV.verts[0]
                : GV.verts[nextFaceIndex * 12 + rr * 4 - 3]
  ...
  isPointInParallelogram(
    P: PositionType,
    [A, B, C, D]: PositionType[],
  ): boolean {
    const ABx = B[0] - A[0]
    const ABy = B[1] - A[1]
    const ADx = D[0] - A[0]
    ...
    if (denom > 0) {
      return u_num >= 0 && u_num <= denom && v_num >= 0 && v_num <= denom
    } else {
      return u_num <= 0 && u_num >= denom && v_num <= 0 && v_num >= denom
    }
  }
  ...
  getHoveredSquare(): SquareID | null {
    if (this.gameplay.phase !== "PLAY" || this.helpModal.isOpened) {
      return null
    } // not play phase || is showing help

    return this.globalHoveredSq
    ...
    // // top face
    // if (
    //   this.isPointInParallelogram(mousePos, [
    //     verts[0],
    //     verts[9],
    //     verts[12],
    //     verts[21],
  ...
  renderGrid() {
    ...
    for (let f = 0; f < 3; f++) {
      ...
      for (let x = 0; x < 4; x++) {
        const v1 =
          x === 0 ? verts[0] : verts[nextFaceIndex * 12 + (x - 1) * 4 + 1]
  ...
  renderBtn(
    ...
    if (prg < 1) {
      ...
      p5.fill(
        p5.lerpColor(p5.color(214, 180, 30), p5.color(this.getSqColor(1)), prg),
      )
    } else {
      p5.fill(this.getSqColor(1))
    }
    p5.rect(0, 0, w, h, 10)
  ...
  pointInRotRect(
    ...
    return (
      Math.abs(dx * cos - dy * sin) <= w * 0.5 &&
      Math.abs(dx * sin + dy * cos) <= h * 0.5
    )
  }
  ...
  isThePlacingSquare(id: SquareID): boolean {
    ...
    for (let i = 0; i < this.animatedPlacingSqs.length; i++) {
      ...
      if (id[0] === sid[0] && id[1] === sid[1] && id[2] === sid[2]) {
        return true
      }
    }
    return false
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let r = 0; r < 3; r++) {
        ...
        for (let rr = 0; rr < 3; rr++) {
          ...
          if (this.isThePlacingSquare([i, r, rr])) {
            continue
          }
  ...
  getSqColor(sd: SquareData): P5.Color {
    if (sd === 1) {
      return this.p5.color(130, 118, 60)
    }
    if (sd === 2) {
      return this.p5.color(245, 228, 44)
    } // golden
    if (sd === 3) {
      return this.p5.color(242, 80, 245)
    } // destroyer
  ...
  renderEndModal() {
    ...
    if (sp === "RATING") {
      ...
      p5.fill(
        ...
        23,
        11,
        sp === "MESSAGE" ? prg * 230 : sp === "DELAY" ? 230 : 230 + prg * 25,
      )
      p5.rect(
        200,
        300,
        400,
        sp === "MESSAGE" ? prg * 150 : sp === "DELAY" ? 150 : 150 + prg * 450,
      )
      customFont.render(
    ...
    if (sp === "PRE-RATING" || sp === "RATING") {
      ...
      if (endModal.rating === RATINGS.length - 1 && endModal.prg > 0.5) {
        endModal.prg = 0.5
      }
      const calculatedPrg = Math.sin(
        Math.PI * Math.max(0, Math.min(1, endModal.prg)),
      )
      p5.push()
      ...
      for (let i = endModal.particles.length - 1; i >= 0; i--) {
        ...
        if (pc.size <= 0) {
          endModal.particles.splice(i, 1)
        }
      }

      // score
      const textWidth = customFont.render(
        endModal.score + "",
        -1000,
        0,
        36,
      ...
      if (endModal.score < gp.goldPoints && p5.frameCount % 2 === 0) {
        ...
        if (endModal.score < 40) {
          endModal.score = Math.min(gp.goldPoints, endModal.score + 1)
        }
        // rating up?
        if (
          endModal.rating < RATINGS.length - 1 &&
  ...
  draw() {
    ...
    if (currentPiece) {
      ...
      if (hoveredSquare) {
        ...
        if (
          this.hoveredBtn === "ROTATE" ||
          this.hoveredBtn === "PLACE" ||
          this.hoveredBtn === "SWITCH"
        ) {
        }
      ...
      if (currentPiece.hoveredSq) {
        const calculatedSqs: this["input"]["calculatedSqs"] = [
          ...
          {
            id: currentPiece.hoveredSq,
            isGolden: currentPiece.op.goldenSqIndex === "CENTER",
          },
        ]

        // all other squares beside center square
        for (let i = 0; i < currentPiece.sqList.length; i++) {
          const id = this.getSteppedSqID(
            currentPiece.sqList[i],
            currentPiece.hoveredSq,
          )
          if (id === null) {
            calculatedSqs.push({
              id: currentPiece.hoveredSq,
            ...
            calculatedSqs.push({
              id,
              isGolden: currentPiece.op.goldenSqIndex === i,
            })
          }
        }
        ...
        for (let i = 0; i < calculatedSqs.length; i++) {
          ...
          if (sq.isOverlapped || sq.isOutOfBound) {
        ...
        if (currentPiece.op.sqList === gp.RAW_PIECES[6]) {
          ...
          for (let i = 0; i < calculatedSqs.length; i++) {
            const sq = calculatedSqs[i]
            if (!facesList.includes(sq.id[0])) {
              facesList.push(sq.id[0])
            }
          }
    ...
    if (gp.phase !== "PLAY") {
      ...
      for (let i = animatedClearingSqs.length - 1; i >= 0; i--) {
        ...
        // check if is placing phase then don't render placing squares
        if (this.isThePlacingSquare(acsq.id)) {
          continue
        }
      ...
      for (let i = animatedSpreadingSqs.length - 1; i >= 0; i--) {
        ...
        // check if is placing phase then don't render placing squares
        if (this.isThePlacingSquare(assq.id)) {
          continue
        }
      ...
      if (gp.phase === "PLACE") {
        ...
        if (gp.placingSubphase === "SLIDE") {
          ...
          if (gp.ug < 1) {
            ...
            gp.ug = 0
            if (this.highestSnapsCount > 1) {
              gp.placingSubphase = "WRAP1"
            } else {
              gp.phase = "SPREAD"
            }
          }
          ...
          // update ug
          if (gp.ug < 1) {
            gp.ug = 1 // Math.min(1, gp.ug + 0.1)
          } else {
            gp.ug = 0
            if (gp.placingSubphase === "WRAP1") {
              if (this.highestSnapsCount > 2) {
                gp.placingSubphase = "WRAP2"
              } else {
                gp.phase = "SPREAD"
              }
            } else {
    ...
    if (gp.phase === "END" && this.goldenLasers.length === 0) {
      if (gp.ug < 1) {
        gp.ug = 1 // Math.min(1, gp.ug + 0.025)
      } else {
        gp.ug = 0
